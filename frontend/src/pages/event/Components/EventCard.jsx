            ...
            <div className="event-card-header">
                <div className="event-card-badges">
                    <span className={`badge ${event.isPaid ? 'badge-paid' : 'badge-free'}`}>
                        {event.isPaid ? (event.ticketTypes?.length > 0 ? `Rs ${Math.min(...event.ticketTypes.map(t => parseFloat(t.price) || 0))}${event.ticketTypes.length > 1 ? '+' : ''}` : (event.ticketPrice ? `Rs ${event.ticketPrice}` : 'Paid')) : 'Free'}
                    </span>
            ...
            <div className="event-card-details">
                ...
                {event.additionalDates && event.additionalDates.length > 0 && (
                    event.additionalDates.map((d, i) => (
                        <div className="event-detail" key={`addl-date-${i}`}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                                <line x1="16" y1="2" x2="16" y2="6" />
                                <line x1="8" y1="2" x2="8" y2="6" />
