...
export const registrationService = {
  ...
  uploadReceipt: async (registrationId, formData) => {
    const res = await api.post(`/registrations/${registrationId}/upload-receipt`, formData, {
  ...
  getDashboardStats: async (eventIds = []) => {
    ...
    const res = await api.get('/registrations/dashboard-stats', { params: { eventIds: eventIds.join(',') } });
    return res.data.data;
  },
  ...
  getPendingRegistrations: async (eventId = null) => {
    ...
    const res = await api.get('/registrations/pending', { params });
    return res.data.data;
  },
  ...
  reviewRegistration: async (registrationId, action, notes = '') => {
    const res = await api.put(`/registrations/${registrationId}/review`, { action, notes });
    return res.data;
  },

  // GET /api/v1/registrations/attendance/:eventId
  getAttendance: async (eventId) => {
    const res = await api.get(`/registrations/attendance/${eventId}`);
    return res.data.data;
  },

  // GET /api/v1/registrations/:id/qr-code
  getQrCode: async (registrationId) => {
    const res = await api.get(`/registrations/${registrationId}/qr-code`);
    return res.data.data;
  },

  // GET /api/v1/registrations/validate-qr/:token
  validateQrCode: async (token) => {
    const res = await api.get(`/registrations/validate-qr/${token}`);
    return res.data.data;
  },
};
